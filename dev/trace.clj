(ns trace
  "DEV-ONLY runtime half of the inspector — a per-request *construction view* plus a *flow* drill-down, read from a ClojureStorm execution recording.
...
(defn summarize
  ...
  (try
    (cond
      (nil? v) {:kind "nil"}
      (db? v) {:kind "db"
               :type "datomic.Database"
               :basis-t (try
                          (d/basis-t v)
                          (catch Throwable _ nil))}
      (string? v) {:kind "scalar"
                   :type "String"
                   :preview (trunc v 80)}
      (number? v) {:kind "scalar"
                   :type (tname v)
                   :preview (str v)}
      (boolean? v) {:kind "scalar"
                    :type "Boolean"
                    :preview (str v)}
      (keyword? v) {:kind "scalar"
                    :type "Keyword"
                    :preview (str v)}
      (instance? java.util.UUID v) {:kind "scalar"
                                    :type "UUID"
                                    :preview (str v)}
      ;; a Hiccup element (vector with an HTML-tag head) — label it <tag>, not [n=N]
      ;; (strip Hiccup's .class/#id shorthand: :a.card#x -> "a")
      (inspector/element? v) {:kind "hiccup"
                              :tag (first (str/split (name (first v)) #"[.#]"))
                              :n (count v)}
...
(def ^:private db-nss
  #{"myapp.recipe.core" "myapp.db.core" "myapp.auth.core" "myapp.admin.core" "myapp.analytics.db"
    "myapp.web.handler"})
...
(defn- read-window
  ...
  (let [tl (ia/get-timeline tid)
        ...
        calls (persistent!
                (reduce
                  (fn [acc [j m]] (if (= :fn-call (:type m)) (assoc! acc (+ start j) m) acc))
                  (transient {})
                  (map-indexed vector slice)))
...
(defn- db-t
  "The meaningful t of a db value.
  Its as-of POINT when it's a time-travel db
...
(defn- nearest-db-basis
  ...
  Uses db-t, so a frame handed an as-of db (e.g. version-history's
...
(defn- collect-db-exprs
  "All Datomic-call exprs in the window (under db-relevant fns), as {:call-idx i :sf <sub-form> :e <expr-entry>}."
  ...
  (->> (for [[i es] exprs-by-call
             ...
             e es
             :let [sf (try
                        (ia/get-form-at-coord (:form-id m) (:coord e))
                        (catch Throwable _ nil))]
...
(defn- primary-data-arg
  "The 'subject' a fn transforms: its first map/coll/db argument (locale, flags, ids are passed as scalars and skipped)."
  [args]
  (first (filter (fn [v] (or (map? v) (coll? v) (db? v))) args)))

(defn- value-morph
  "How a fn reshaped its primary data arg into its return — {from to} ValueRefs (a type morph like db→map or map→hiccup reads at a glance), plus a key :diff (added/removed/changed) when both are maps.
  This is the 'what did this fn DO'
  signal, computed from the recorded arg+ret."
  [args ret]
  (when-let [a (primary-data-arg args)]
    (cond-> {:from (summarize a)
             :to (summarize ret)}
      (and (map? a) (map? ret)) (assoc :diff
                                  (try
                                    (let [ak (set (keys a))
                                          rk (set (keys ret))]
                                      {:added (mapv str (take 8 (set/difference rk ak)))
                                       ...
                                       :changed (mapv str
                                                      (take 8
                                                            (for
                                                              [k (set/intersection ak rk)
                                                               :when (not= (get a k) (get ret k))]
                                                              k)))})
                                    (catch Throwable _ nil))))))

(defn- defn-src
  "The fn's definition site as a \"file:line:col\" string (the inspector's data-myapp-src format), from FlowStorm's form registry — so even frames with no DOM node (handlers, domain, db) are navigable to their source."
  [form-id]
  (let [f (try
            (ia/get-form form-id)
            (catch Throwable _ nil))]
    (when (and f (:form/file f) (:form/line f)) (str (:form/file f) ":" (:form/line f) ":1"))))

(defn- call-parent
  "The frame that LEXICALLY called `i`: nearest ancestor that isn't the inspector's instrument-var wrapper / tag-tree machinery.
  Unlike build-spans' eff-parent this
...
(defn- call-site
  "Where frame `i` was actually INVOKED — {:src \"file:line:col\" :form <str>}.
  FlowStorm
  ...
  (when-let [a (call-parent calls i)]
    (let [;; w = the frame directly under a on the path to i (the wrapper, or i itself)
          ...
          sf (when site
               (try
                 (ia/get-form-at-coord aform (:coord site))
                 (catch Throwable _ nil)))
          af (try
               (ia/get-form aform)
               (catch Throwable _ nil))
...
(defn- nn-ancestor
  "Nearest NON-noise ancestor frame index of i (skips the inspector's wrapper frames and loop machinery), or nil."
  [calls i]
  (loop [p (:parent-idx (calls i))]
    (cond (nil? p) nil
          (not (calls p)) nil
...
(defn- folded?
  "A DIRECT same-name self-delegation — a multi-arity arity-default ((f a b) -> (f a b nil)) or tail self-recursion — whose nearest non-noise ancestor is the same fn.
  It's a dispatch artifact, not distinct structure: the
...
(defn- build-spans
  ...
  (let [kept? (fn [i]
        ...
        ops-by-call (->> (collect-db-exprs w)
                         (keep (fn [d]
                                 (when-let [k (kept-anc (:call-idx d))] ; re-attribute folded ops upward
                                   (assoc d
                                     :call-idx k
                                     :op (->db-op calls d)))))
                         (group-by :call-idx))
        ...
        throw-idx (when (and origin-uw (calls (:fn-call-idx origin-uw)))
                    (kept-anc (:fn-call-idx origin-uw)))
        throw-info (when throw-idx
                     (let [fnidx (:fn-call-idx origin-uw)
                           t (:throwable origin-uw)
                           sf (try
                                (ia/get-form-at-coord (:form-id (calls fnidx)) (:coord origin-uw))
                                (catch Throwable _ nil))
                           af (try
                                (ia/get-form (:form-id (calls fnidx)))
                                (catch Throwable _ nil))]
                       (cond-> {:type (try
                                        (.getName (class t))
                                        (catch Throwable _ "?"))
                                :msg (try
                                       (trunc (.getMessage ^Throwable t) 200)
                                       (catch Throwable _ nil))}
                         sf (assoc :form
                              (trunc (form->str sf) 110) :at
                              (str (:form/file af)
                                   ":" (or (some-> sf
                                                   meta
                                                   :line)
        ...
        ->span (fn [i]
                 (let [m (calls i)
                   ...
                   (cond-> {:id (str i)
                     ...
                     src (assoc :src
                           src)
                     (:src cs) (assoc :call-src
                                 (:src cs) :call-form
                                 (:form cs))
                     inst (assoc :instance
                            inst) ; 0 is truthy in Clojure — fine
                     lz (assoc :lazy
                          true)
                     rb (assoc :realized-by
                          (short-name (frame-name (calls rb))) :realized-by-id
                          (str rb))
                     (= i throw-idx) (assoc :threw
                                       throw-info)
                     mr (assoc :morph
                          mr)
                     (seq ops) (assoc :db-ops
                                 ops))))
        children (reduce
                   (fn [acc i]
                     (if-let [p (tree-parent i)]
                       (update acc p (fnil conj []) i)
                       acc))
                   {}
                   kept-idxs) ; lexical (re-parented)
        children-rt (reduce
                      (fn [acc i]
                        (if-let [p (eff-parent i)]
                          (update acc p (fnil conj []) i)
                          acc))
                      {}
...
(defn- detect-repeats
  "Cross-frame N+1 signal: the same query FORM run ≥2× in one request (a query in a `for`, or redundant lookups across middleware).
  Returns [{form count op where}] —
  ...
  (let [w (read-window tid start end)
    ...
    {:id id
     :request {:method (some-> method
               ...
               :uri uri
               :status status
               :ms (when ms (Math/round (double (* ms 100))))
               :ms-scale 100}
     :recording? (recording?)
...
(defn- result-has-eid?
  ...
  (try
    (let [hit? (fn [x]
                 (or
                   (= x eid)
                   (and (map? x) (= eid (safe-eid x)))
                   (and (sequential? x) (some #(= % eid) x))))]
      (cond (nil? eid) false
...
(defn flow
  ...
  (let [{:keys [calls exprs-by-call at]
         :as w}
        (read-window tid start end)
        named (->> (keys calls)
                   sort
                   ...
                   (filterv (fn [i]
                              (and (not (folded? calls i))
                                   (inspector/element? (some-> (:ret-idx (calls i))
                                                               at
                                                               :result))))))
    ...
    (cond
      ...
      (let [m (calls target)
                   ...
                   (if-let [mk (calls k)]
                     (recur
                       ...
                       (if (noise? mk)
                         ...
                         (conj
                           acc
                           {:short (short-name (frame-name mk))
                            :name (frame-name mk)})))
                     (vec (reverse acc))))
            value (when src-line
        ...
        {:component comp-name
         ...
         :span {:short (short-name comp-name)
                :name comp-name
                :args (mapv summarize (:fn-args m))
                :ret (summarize
                       (some-> (:ret-idx m)
...
(defn get-trace-json
  ...
  (when-let [d (get-desc id)]
    (try
      (json/write-value-as-string (project d))
      (catch Throwable e
...
(defn get-flow-json
  ...
  (when-let [d (get-desc id)]
    (try
      (some-> (flow d comp-name idx src-line)
              json/write-value-as-string)
...
(defn get-branches-json
  "Conditionals in a FRAME that evaluated falsy → their body was NOT rendered ('why is this section empty?').
  {:skipped [{op form line}]}. For /dev/__branches."
  [id frame]
  (when-let [d (get-desc id)]
    (try
...
(defn get-source-json
  "The DB read(s) that produced the entity a FRAME received — 'where did this frame's data come from' — keyed by frame idx (for the details-pane dossier).
  ...
  (when-let [d (get-desc id)]
    (try
      (let [{:keys [calls]
             :as w}
            (read-window (:tid d) (:start d) (:end d))
            m (calls frame)
          ...
          (if-not eid
            ...
            {:eid eid
             :reads (->> (collect-db-exprs w)
                         (map (fn [dx]
                                (assoc (->db-op calls dx)
                                  :source (result-has-eid? (:result (:e dx)) eid))))
                         (filter :source)
                         vec)})))
      (catch Throwable e (json/write-value-as-string {:error (str (.getMessage e))})))))
...
(defn- slot-value
  ...
  (when-let [m (calls frame)]
    (cond
      (= slot "ret") (some-> (:ret-idx m)
                             at
                             :result)
      (str/starts-with? (str slot) "arg")
      (when-let [n (parse-long (subs (str slot) 3))]
        (nth (vec (:fn-args m)) n nil)))))

(defn- child-at
  "The i-th child of a collection, in the SAME seq order expand-value enumerates (so a path of indices navigates deterministically)."
  [v i]
  (cond
    (map? v) (some-> (nth (seq v) i nil)
                     val)
...
(defn- expand-value
  "One bounded level of a value: its children (key/index -> ValueRef, expandable?) and, for a coll-of-maps, a table."
  [v]
  (let [children (cond
                   (map? v) (vec
                              (map-indexed
    ...
    (cond-> {:ref (summarize v)}
      children (assoc :children
                 children :truncated
                 (boolean (and (counted? v) (> (count v) 50))))
      table (assoc :table
              table))))

(defn get-value-json
  "Expand the recorded value at frame/slot navigated by `path` (a vector of child indices).
  For /dev/__value."
  [id frame slot path]
  (when-let [d (get-desc id)]
    (try
      (let [w (read-window (:tid d) (:start d) (:end d))]
...
(defn- hiccup-tree
  "A recorded Hiccup value as a compact element tree.
  Flattens seq bodies
  ...
  (cond
    ...
    (let [head (name (first v))
      ...
      (cond-> {:kind "el"
               :tag (first (str/split head #"[.#]"))
               :sel head}
        (:class attrs) (assoc :class
                         (trunc (str (:class attrs)) 70))
        (:data-myapp-name attrs) (assoc :comp
                                   (:data-myapp-name attrs))
        (:data-myapp-src attrs) (assoc :src
                                  (:data-myapp-src attrs))
        (:data-value attrs) (assoc :val
                              (str (:data-value attrs)))
        (seq kids) (assoc :children
                     kids)))
    (string? v) {:kind "text"
                 :text (trunc v 90)}
    (or (number? v) (keyword? v)) {:kind "text"
                                   :text (str v)}
    (nil? v) nil
    (and (seq? v) (not (vector? v)))
    {:kind "seq"
     :children (when (< depth 8)
                 (->> v
                      (remove nil?)
                      (take 40)
                      (mapv #(hiccup-tree % (inc depth)))))}
    :else {:kind "other"
           :preview (trunc (bounded-pr v) 60)}))
...
(defn- value-occurrences
  "Frames where `target` flows in as an arg or out as the ret — by object identity (structural sharing threads the SAME map) or by :db/id for entities."
  [{:keys [calls at]} target]
  (let [teid (when (map? target) (safe-eid target))
        same? (fn [v]
                (try
                  (or (identical? v target) (and teid (map? v) (= teid (safe-eid v))))
                  (catch Throwable _ false)))]
    (when (some? target)
      (for [i (sort (keys calls))
            :let [m (calls i)
                  ...
                  slots (vec
                          (concat
                            (keep-indexed (fn [j a] (when (same? a) (str "arg" j))) (:fn-args m))
                            (when (same? ret) ["ret"])))]
            :when (seq slots)]
        {:frame (str i)
         :slots slots}))))
  ...
  (when-let [d (get-desc id)]
    (try
      (let [w (read-window (:tid d) (:start d) (:end d))
        ...
        (json/write-value-as-string
          {:ref (summarize target)
           :frames (vec (value-occurrences w target))}))
      (catch Throwable e (json/write-value-as-string {:error (str (.getMessage e))})))))
...
(defn- page-request?
  "Is this request something that becomes (part of) the screen — and so worth recording?
  Covers a top-level navigation, a form submit, AND the dispatcher's
...
(defn- full-navigation?
  "A genuine top-level page load from scratch — address bar, hard refresh, back/forward, a non-enhanced link, a full form submit.
  Browsers tag these
...
(defn- record-page
  "Record a page/morph render, then stop recording so assets, /dev/ reads, and idle accrete nothing.
  On a FULL navigation (not an in-page morph) first discard
  ...
  [handler req]
  (when (full-navigation? req) (clear-recordings!)) ; new page from scratch — evict the prior visit (bumps epoch)
  (enable-recording!)
  (let [tid (thread-id)
          ...
          (let [id (str (java.util.UUID/randomUUID))]
            (put!
              ...
              {:id id
               ...
               :ms ms})
            (-> resp
                (assoc-in [:headers "X-Myapp-Trace"] id)
                (update :body stamp-html id)))
          resp))
      (catch Throwable e
...
(defn get-last-error-json
  "The most recent uncaught-error trace (id + uri + exception), or null.
  For /dev/__last-error — lets a successful page surface the prior failed request."
  []
  (let [le @last-error]
