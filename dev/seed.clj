...
(defn seed!
  "Re-seed the operational database with recipes.
  Idempotent-ish: it just adds
...
(defn seed-if-empty!
  ...
  (let [n (or (d/q '[:find (count ?e) . :where [?e :recipe/id]] (db/get-db)) 0)]
    (if (zero? (long n))
      (do (println "Empty dev database — seeding demo recipes...") (seed!) true)
      (do (println (format "Dev database has %d recipe(s) — skipping seed." (long n))) false))))
