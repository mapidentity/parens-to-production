...
One honesty note, and it is the same one [the scaling audit](41-beyond-one-box.md) made about the transactor's failover: this ruleset is *reviewed against the nftables grammar, not applied in the book's sandbox*, which lacks the kernel capability to load packet rules. Applying and testing it — `nft -f`, then a port scan from off-box confirming only 80/443 answer — is a step for the real host, and the runbook says so. A firewall you have not port-scanned from outside is a firewall you are trusting; this chapter is honest about which claims it drilled and which it reasoned.
...
`clojure -M:nvd "" "$(clojure -Spath)"` now resolves and runs — verified to the point where the tool loads and demands its classpath argument, which the CI invocation supplies. The full scan downloads the NVD data feed on first run, which is genuinely slow and rate-limited without an `NVD_API_KEY` — exactly the caveat chapter 34 already stated, and the reason the feed is cached in CI rather than fetched per build. What matters for the audit is that the gate is no longer vapor: the alias exists, the tool is on the classpath, and [the pipeline](34-ci-cd.md) has a real step to run rather than a paragraph to cite.

The gate has a blind spot worth naming precisely, because it motivates the next section: **it scans the application's Java classpath, and nothing else.** The kernel, `openssl`, `curl`, the C libraries the JVM links — the CVE that lands there is invisible to `nvd-clojure`, because that is not what it looks at.
...
The last surface is the process itself: if the app is compromised, how much of the box does the attacker inherit? systemd answers with sandboxing, and [the going-live units](35-going-live.md) carried a starter set — `NoNewPrivileges`, `PrivateTmp`, `ProtectSystem`. This chapter completes it across every unit:
...
The book opened by betting that an application built for correctness, security, and the daily experience of working on it would end up not harder to own but simpler — and that the proof would be the whole of it standing together. It now does: from an empty directory to a box that renders in a millisecond, deploys without a visible seam, watches itself, calls for help, restores from disaster, forgets on demand, and — with these last chapters — sees an attacker, answers, and keeps its surface small and current. No layer taken on faith; no decision that cannot be re-made; and every security control either drilled or honestly named as reasoned. That is what owning your code all the way down means, and it is where the wager comes to rest. The afterword has the last word on where you take it.
