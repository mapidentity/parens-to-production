...
It reuses two things from [the live-reload chapter](06-live-reload.md) -- the file watcher and the `dev-reload` WebSocket -- and hooks into the `base-layout` from [the Hiccup views chapter](12-hiccup-views.md). Everything here is dev-only and **structurally absent** from production builds, the same as the rest of our dev infrastructure. If you read strictly in order, the Hiccup views chapter's layout sections are the relevant background.
