...
For this application the clock is not incidental. Two of its features lean on time directly: magic-link tokens that are valid for fifteen minutes and not a second longer ([the auth-tokens chapter](20-auth-tokens.md)), and the point-in-time history that is the whole point of a versioning site -- "show me this recipe as it stood at that moment." A feature whose correctness *is* a statement about time needs a clock it can pin, not one it reads off the wall.
