...
`:has()` does real work here too. The dashboard's drag-reorder feel -- siblings gliding aside, the dragged row lifting and tilting, the rest receding -- is expressed almost entirely in CSS, with JavaScript reaching in only to set a single custom property:
...
The flagship controller is `sortable`, and it is where the chapter's thesis gets its sharpest test. Dragging *needs* JavaScript: there is no CSS that reads a pointer, so a zero-JS drag-reorder does not exist. But the **motion** does not need JavaScript, and that is the part worth obsessing over. The modern technique is to stop hand-writing FLIP and let the View Transitions API do the choreography; the controller's whole job shrinks to translating "the pointer is here" into things CSS can act on.
