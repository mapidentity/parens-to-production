...
Two cheaper moves are worth weighing before you provision that store, because each covers *part* of the problem with no new infrastructure. You can sticky-hash the client at [the load balancer](36-minimal-downtime.md) -- consistent-hash on IP, so a given address always lands on the same instance -- and each process's in-process counter becomes authoritative for the slice of the keyspace it owns; one abusive IP concentrating on one process is a *feature* here, not a hot-key problem. Or push the throttle to [the edge](35-going-live.md) outright: Caddy and nginx rate-limit by client address natively, before a request reaches any instance, so for that dimension the cross-instance problem never arises. What neither can touch is the per-*email* dimension. The address lives in the POST body, and a load balancer routes on connection metadata it can see, not a form field it cannot, so it can neither partition nor count by email. That one dimension is what makes the shared store non-negotiable -- the reason the fix here is `allow?` behind a store rather than a routing rule you could have written in the Caddyfile.
