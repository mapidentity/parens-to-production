...
One more honest cost, and it is the mirror image of the upside. Because the cookie authenticates *itself* and the server keeps nothing, there is nothing on our side to revoke. A cookie that leaks -- copied off a shared machine, lifted from a backup, pulled from a proxy log -- stays valid for the rest of its thirty-day life, and no action of ours cuts it short: logout runs `(assoc :session nil)` on the *requesting* browser and clears that one cookie, but it cannot reach a copy already made, there is no "sign out of all my devices," and the only global lever is rotating `SESSION_KEY`, which fails every existing cookie at once and signs the whole userbase out. A server-side session store is exactly what would buy back the power to strike one session dead; declining the store declines the revocation with it. That is the trade the "no session table" line above is really making, and it belongs in the open rather than under the word *ideal*.

The gap is narrower than it first looks, though, because one half of revocation is already free and the other half is cheap. `wrap-current-user` resolves the cookie's email to a live user entity on *every* request, so authority is never staler than the last read: delete a user and their next request arrives with no `:user-eid`, which `wrap-auth` treats as unauthenticated and clears -- deletion is already a working kill switch. Disabling short of deletion is one attribute away, and the schema already carries it -- `:user/active?` is set on every account and shown on the admin dashboard, yet nothing on the request path *reads* it. Gate the resolve in `wrap-current-user` on that flag and a single flip becomes a real ban, still with no store to keep. Only true per-session revocation -- keep this laptop, kill that phone -- needs more: a `:user/sessions-valid-after` instant stamped into the session at login and compared on read, so "sign out everywhere" becomes one write. This app ships none of them, because a recipe box does not need them; what matters is that the sealed cookie forecloses none of them either, and each has an obvious home in the lookup `wrap-current-user` already does.
...
A request lands here in one of three states -- not authenticated (redirect home), authenticated without terms (redirect to `/terms/welcome`), authenticated with terms accepted (render the dashboard) -- and the nested `if`s test them in exactly that order.
