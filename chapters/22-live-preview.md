...
Note what the initial pane content does for the no-JS story. The `aside` is server-rendered *with content*: an edit page shows the stored recipe's body before a single keystroke -- and, with JavaScript absent entirely, continues to show it: a truthful, static rendering of what is saved, sitting beside the form that edits it. Layer 0 does not get the *live* in live preview; it gets a poorer version of the same page, not a broken one, which has been [the layering's promise](20-progressive-enhancement.md) since it was drawn.
