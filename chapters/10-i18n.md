...
Because we do not need those features yet. The entire i18n machinery -- `t`, `detect-locale`, `wrap-locale`, and the locale vars -- is a few dozen lines of code, before the translation maps themselves. It handles two languages, falls back gracefully, auto-detects from the browser, and hot-reloads during development. When we need pluralization or interpolation, we will either add a few more lines to `t` or pull in a library at that point.
