...
The conversion wrappers are thin -- about 40 lines. They are not a framework or an ORM; they solve one specific problem, the `Date`/`Instant` mismatch, and stay out of the way for everything else. You still write Datalog directly, navigate with `d/entity`, and transact plain maps through `transact*`. Datomic's API is good; it just needs this one bridge -- and with it in place, the chapters that follow build domain logic on top without ever seeing a `java.util.Date`. (The same `myapp.db.core` namespace later grows a second, unrelated concern: a tenant-isolation layer -- the `*-owned` helpers that enforce per-user data access -- which we reach for once there are users to isolate, in [the progressive-enhancement chapter](18-progressive-enhancement.md). It is not part of the bridge, so it is not in that 40-line count.)
