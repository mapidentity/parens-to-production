...
`flow` ties them together: resolve the instance, walk its ancestor path, flag the reads that produced its entity, and read the clicked line's recorded value if a source line was passed. It reuses `read-window`, `folded?`, `collect-db-exprs`, and `->db-op` from chapter 17:
