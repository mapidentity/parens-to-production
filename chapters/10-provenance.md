...
Note what these attributes are *not*: they are not in `pull-pattern`, not in the diff's field list, not anywhere a recipe read can see. Provenance rides beside the data, never inside it. Every failure mode the reflex fix had is structurally impossible here.
...
- **Deletion provenance is recorded but not yet surfaced.** A delete's transaction knows who did it; no current page shows deleted recipes, so the fact waits. It is exactly the material an admin audit view would be built from -- [the log was the audit table all along](26-activity.md).
