...
Three changes make the backup a *watched* system:

- **Re-verify it, on a timer.** `myapp-backup-verify.timer` runs the drill weekly -- restore the newest archive into throwaway storage, then prove it is usable, not merely present: entities are there *and* `d/history` is populated, because the [time axis is the thing this product is about](39-backup-restore.md) and a restore that loses it is a restore that lost everything that matters. Any failure pages on both channels. The drill is no longer something a human remembers; a timer proves it, and stamps its `verdict:"PASS"` into the manifest below.
- **Make the run legible.** Every backup appends one line to a [manifest](39-backup-restore.md) -- status (written on failure too, by a trap), the basis-t it captured, the photo count, whether the off-box file copy actually went. A backup stops being a file whose mtime you squint at and becomes a record you read.
- **Watch the record, not the mtime.** The watchdog now fails if the last manifest record is not `status:"ok"`, or is older than 25 hours, or reports a *failed* off-box copy. That catches three things a "newest file is recent" check cannot: a run that crashed half-written, a run that never happened, and a run whose databases went off-box while its photos silently did not -- the last invisible to any check that only stats the local directory.
...
The last watcher to watch is the operator. This chapter gave you a loopback REPL so the runbook's live levers (ban a user, inspect the registry, repair a bad write) actually exist to be pulled. That REPL is unauthenticated code execution against the live process. It is the most powerful tool in the book and the most dangerous, and the two are the same sentence.
