...
>
> *Scope -- and it is architectural, not a line count:* htmx is *element-granular* hypermedia: any element can issue any request and swap any target. An app that uses it the way its ecosystem intends grows a server API shaped like fragments: handlers returning partials aimed at swap targets, a second answer to what an endpoint is. This app's contract is *page-granular*: the server renders complete documents, and enhancement reaches the server at only two small endpoints (a plain-text `/partials/tagline` endpoint built in [the progressive-enhancement chapter](20-progressive-enhancement.md), and the admin dashboard's JSON `/stats` poll), neither of which is the fragment-shaped HTML API an htmx codebase grows toward, and the client behavior amounts to intercept-fetch-morph-`<main>` plus islands the controller registry already owns. Used at page granularity, htmx collapses to `hx-boost` plus a morph extension, at which point it is the couple hundred lines we would rather own outright.
>
