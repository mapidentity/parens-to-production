...
The [previous chapter](14-hiccup-views.md) ended on a deliberate omission. Every handler renders the complete page and never branches on how it was called; every page's content sits inside a single `<main data-layout>`. That leaves one question hanging: if the server always sends a whole page, what makes navigating between pages feel instant instead of like a full reload? This chapter is the answer, and it is one script.

That script is the *dispatcher*, loaded once by the base layout as an ES module. It turns ordinary links and forms into smooth, in-place updates with no per-element configuration and no change to how the server responds. And if it never loads, every link still navigates and every form still posts. It is worth being precise about its scope, because it is narrow on purpose: morphing is the right tool for one kind of update (reconciling server-authored HTML), and it is *not* the whole progressive-enhancement story. Menus, motion, and local interactions each have their own narrower correct mechanism, and choosing among them is the subject of [the progressive-enhancement chapter](20-progressive-enhancement.md), which treats this dispatcher as the top layer of a five-layer stack. Here we build that layer.
...
So cmd-click to open in a new tab, downloads, external links, and in-page `#anchor` jumps all behave as the browser intends. The only opt-out attribute is `data-no-enhance`; everything else is inferred from the link itself.
...
Two options in the skeleton's `morph` call pass straight through to idiomorph. `ignoreActiveValue` tells it to leave the focused element's value alone; the live-preview enhancers of [the progressive-enhancement chapter](20-progressive-enhancement.md) set it so the field you are typing into survives its own round-trip. `morphCallbacks` forwards an idiomorph callbacks object and exists for exactly one caller, the dev hot-reload of [the morph-reload chapter](19-morph-reload.md), which uses it to shield client-injected dev-overlay nodes from removal.

## A note on the accept header

You will see the fetch send `headers: { 'Accept': 'text/html' }`. **The server does not branch on it.** There is no content negotiation, no "partial vs full" response, no special header that flips the handler into a different mode. Every handler the dispatcher fetches renders the complete page every time; the dispatcher is the only thing that decides to extract `<main>`. (The app does have a few purpose-built enhancement endpoints -- the tagline partial, the admin stats poll, the live-preview pane -- but those are their own routes, not a page handler flipped into fragment mode by a header.) The `Accept` header is vestigial -- harmless, but do not build server logic around it. Keeping the server oblivious to *how* it is being called is what keeps the no-JavaScript path correct without extra work.
...
(At the repo head the `detail` carries a fourth field, `traceId` -- dev-only plumbing that [the construction-view overlay chapter](18-construction-view-overlay.md) adds so its overlay can trace a morphed region; in production it is `null`.)
...
>
> *Scope -- and it is architectural, not a line count:* htmx is *element-granular* hypermedia: any element can issue any request and swap any target. An app that uses it the way its ecosystem intends grows a server API shaped like fragments: handlers returning partials aimed at swap targets, a second answer to what an endpoint is. This app's contract is *page-granular*: the server renders complete documents, and enhancement reaches the server at a small, fixed set of dedicated endpoints -- a plain-text `/partials/tagline` built in [the progressive-enhancement chapter](20-progressive-enhancement.md), the admin dashboard's JSON `/stats` poll, the live markdown-preview pane, and the SSE viewer stream of [the live-presence chapter](45-live-presence.md). The preview pane does hand back a fragment of HTML; the difference from htmx is that this is a bounded handful of purpose-built routes, not a contract where every handler grows a partial aimed at a swap target. The rest of the client behavior amounts to intercept-fetch-morph-`<main>` plus islands the controller registry already owns. Used at page granularity, htmx collapses to `hx-boost` plus a morph extension, at which point it is the couple hundred lines we would rather own outright.
>
> *The CSP:* our no-`eval`, hash-based policy is the strictest posture the asset pipeline can express, and htmx's attribute-expression features (event filters, `js:`-prefixed values) need the `eval`-like evaluation a no-`unsafe-eval` policy refuses. Those features can be configured off, but then the guarantee rests on configuration; ours rests on construction, a few pages of our own ES modules hashed like everything else. *Ownership:* the dispatcher's entire surface reads in one sitting, and [the morph-reload chapter](19-morph-reload.md) and [the progressive-enhancement chapter](20-progressive-enhancement.md) both extend it in ways that are only cheap because we own the seam. (We already vendor the one piece of that family we could not improve on: idiomorph is the htmx project's own morphing engine.)
>
> In the Clojure world the decision often arrives pre-made. **Biff** and **Kit** (the community defaults [the positioning chapter](02-positioning.md) named) both scaffold this SSR + htmx shape, and **Datastar** (newer, server-authoritative, SSE-streamed, with an official Clojure SDK) makes the same bet from the signals end. They are good answers whose trade is the inverse of ours (a maintained vocabulary in exchange for a seam you do not own), and the architecture of this book survives the swap either way, because the server-side contract (render whole pages, one morph target) is identical.
...
That narrow surface is also why `fetchAndMorph` turns out to be reused well beyond navigation. The dev hot-reload loop in [the morph-reload chapter](19-morph-reload.md) is, as that chapter puts it, just one more caller of this same function: a view edit on the server becomes a morph in the browser through this path, with no dev-only morphing code to maintain. And the entire island layer in [the progressive-enhancement chapter](20-progressive-enhancement.md) hangs off the one event this function fires. Build the dispatcher once, correctly, and two later chapters get to treat it as bedrock.
