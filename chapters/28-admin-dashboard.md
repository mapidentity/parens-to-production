...
- **One admin, gated by one configured email.** The solo-operator assumption is baked in: the whole privileged surface opens to a single address, and the case-insensitive fold is what keeps a stray capital from locking that one person out. More than one admin, or roles, would mean replacing the equality check with real authorization.
- **The analytics database is disposable, with one string attached.** It holds the magic-link nonces, so deleting and recreating it drops every nonce record; `consume-nonce!` then fails closed on a nonce it cannot find, so any magic link outstanding at that instant stops verifying and its owner must request a fresh one. Schedule a wipe only when no unexpired links are in flight.
- **`total-users` counts entities, not completed signups.** A user entity is created at email verification, before terms acceptance, so `total-users` is a superset of "Terms Accepted" and reads higher whenever someone verified but never accepted. The funnel's `terms-accepted` is the number that means "finished signing up."
- **Live polling is a 20-second `setInterval`, not push.** No SSE, no WebSocket: the numbers can be up to twenty seconds stale, and each open dashboard polls the privileged `/admin/stats` endpoint on its own timer. That is deliberately the cheapest thing that works for a single operator; a real-time feed would be a different mechanism.
- **The queries read across everything.** `all-users` pulls every user, and `recent-magic-links` sorts the whole nonce log before taking 50; both are bounded by a solo-SaaS catalog today and would need pagination or indexed reads at scale.
