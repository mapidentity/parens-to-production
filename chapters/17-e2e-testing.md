...
We keep `:same-site :lax` to match production -- the magic-link flow is a cross-context GET, and `:strict` would block the cookie on that navigation (more on this in [the email login-flow chapter](16-auth-email-flow.md)). We do drop `:secure`, because the e2e server runs over plain HTTP on `localhost`; a `:secure` cookie would never be sent. Those are the only deliberate cookie differences.
