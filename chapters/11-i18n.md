...
With the middleware in place, every handler receives a request with a `:locale` key. Handlers pass it to view functions, and views use `t` to look up strings. Here is a simplified version of the `home-page` we build in [the Hiccup views chapter](13-hiccup-views.md) -- the layout chrome (`public-layout`, logo, Tailwind classes) is stripped out so the `t` calls stand out:
