...
The metrics *format* war is over and the text won: a Prometheus exposition is `name{label="value"} 1.0`, one sample per line. That is not a thing this book buys a client library for -- the whole of `myapp.web.metrics` is a `StringBuilder` and three sources. The JVM's, first: heap, GC, threads, straight from the `ManagementFactory` MXBeans. The request path's, second, folded in by one middleware sitting just inside [the panic belt](25-auth-email-flow.md):
...
- **Sums and counts, no histograms.** Mean latency per status class is deliberately as far as this goes: percentiles need buckets, buckets need choices, and [ch.32's](32-server-path-measured.md) offline distribution already characterizes the shape. Add buckets when a live percentile would change a decision.
