...
- **Poll-on-visit, not push.** News arrives when you show up, which is what "while you were away" means. Real-time delivery is a different feature wearing the same data: [an SSE island](20-progressive-enhancement.md) running the same `activity` query on a heartbeat would be the architecture-honest version, and [the positioning chapter](02-positioning.md) already priced that road. Email digests remain where the afterword put them: with the jobs infrastructure this book deliberately does not build.
