...
- **Propose to the parent only.** A proposal targets the recipe the fork descends from, not any recipe. Generalizing (propose across the lineage, or to a sibling) is a schema that already stores `target` explicitly for exactly that future; the authorization and merge are unchanged.
- **Notifications are pull, not push.** The target owner sees pending suggestions when they visit the recipe. Surfacing them on the dashboard or via [the activity feed](26-activity.md) is a read away — the proposals are queryable — and a real-time nudge is the *next* chapter's territory, not this one's.
