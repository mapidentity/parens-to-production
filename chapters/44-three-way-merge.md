...
- **Notifications are pull, not push.** The target owner sees pending suggestions when they visit the recipe. Surfacing them on the dashboard or via [the activity feed](26-activity.md) is a read away — the proposals are queryable — and a real-time nudge waits for the *next* chapter.
