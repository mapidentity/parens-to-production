...
A ValueRef renders as a short string with a preattentive colour per kind — the data-viz principle that *type is categorical*, so it gets a hue, not a shape:
...
A tall tree is hard to read at a glance, so the panel header gets an **icicle** — a flame-graph cousin. One cell per frame, partitioned top-down, **width ∝ subtree size** and **colour ∝ architecture layer**. You read the request *shape* in one look: the middleware stack narrowing into the handler, a narrow db column beside the wide views subtree. It is an *overview* on top of the working tree — which is why we add it third, not first.
...
- **The recorder is unbounded.** FlowStorm's per-thread timeline grows for the life of the dev JVM. Our descriptor ring buffer caps only the descriptors; reclaim the recording with `(trace/clear-recordings!)` / `/dev/__trace-clear` / a REPL restart.
...
- It rides the same toggle as the inspector — `Alt+Shift+I` (or the corner badge) brings up both at once. With them up, the inspector still answers *where is this element from?* while the construction view answers *how was this whole page built?* — the middleware-to-handler-to-domain-to-view call tree (as an icicle overview and a navigable tree, in lexical or temporal order), each frame's return, the value transforms, and every Datomic read as real datalog at its basis-t, including the raw `d/as-of`/`d/history` time-travel reads.
- A persistent **dossier** for any selected frame: its call site and definition, navigable args and return, the queries it ran, the conditionals that *didn't* render, the markup it produced, the entity it received and the read that produced it, and a value-thread that lights every frame a value flows through.
...
- **N+1 detection, exception capture, and a prior-error banner** fall out of the same recording — no new instrumentation.
- **Per-region traces** keep all of this correct across idiomorph partial updates and dev hot-reloads.
