...
- **A held connection per viewer.** SSE keeps a socket open per watcher; http-kit carries many cheaply, but an unbounded public endpoint that holds connections is a denial-of-service surface, and [connection limits belong at the proxy](34-ci-cd.md), not in this handler. Named, not defended.
