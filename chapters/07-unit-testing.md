...
The same pattern applies to the analytics database, if your app has one. It uses the `analytics` alias from the `:require` above (`myapp.analytics.db`, which we build in [the admin dashboard chapter](16-admin-dashboard.md) -- omit both the require and this fixture until then):
