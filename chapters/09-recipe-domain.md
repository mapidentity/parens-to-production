...
Everything the chapter builds is already decided in this listing, and the decisions are worth naming before the code that depends on them. (One trim to flag: the repository carries longer docstrings on `recipe-schema`, `schema`, and `:recipe/position` than the listing shows; the attribute definitions themselves are identical.)
