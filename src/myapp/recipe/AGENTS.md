...
- `line-diff` is Myers' O(ND) diff (git's algorithm): cost tracks the EDIT DISTANCE, so a small edit to a big recipe is near-linear — no line-count cap needed. The one bound is `max-edit-distance` (1500): a pair sharing almost nothing degrades to `coarse-diff` (all-del/all-add). Heap is O(D·ceiling) — the frontier snapshots are full width, not trimmed to [-d,d], so the pathological peak is ~36MB (not the O(D²) a trimmed trace would give, nor git's O(n) linear-space refinement). Output orders a replaced line as del-then-add (git hunk order). Keep the primitive `long`-array frontier.
